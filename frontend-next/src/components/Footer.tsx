          ...
          <div>
            ...
            <ul className="space-y-2 text-sm text-muted-foreground">
              {["Москва", "Санкт-Петербург", "Казань", "Сочи", "Екатеринбург"].map((city) => (
                <li key={city}>
                  <Link href={`/courts?city=${encodeURIComponent(city)}`} className="hover:text-primary transition-colors flex items-center gap-1">
                    <MapPin size={12} />
                    {city}
                  </Link>
                </li>
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm text-muted-foreground">
              {["Что такое падел", "Правила игры", "Выбор ракетки", "Школы и секции", "Турниры"].map((item) => (
                <li key={item}>
                  <Link href="/news" className="hover:text-primary transition-colors">{item}</Link>
                </li>
              ))}
            </ul>
          </div>
